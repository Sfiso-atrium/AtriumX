...
REVOKE ALL ON FUNCTION create_accommodation_listing(uuid, text, integer, text, text, text[], text[], text[], text, jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION create_accommodation_listing(uuid, text, integer, text, text, text[], text[], text[], text, jsonb, text) TO authenticated;
