...
export default function ListingDetail() {
  ...
  useEffect(() => {
    ...
    getListingById(id).then(l => {
      setListing(l as any)
      if ((l as any)?.user_id) getPublicProfile((l as any).user_id).then(setSeller)
      setLoading(false)
  ...
  const images = (listing as any).image_urls?.length ? (listing as any).image_urls : ['/images/placeholder-listing.png']
  const isOwn = currentUser?.id === (listing as any).user_id
      ...
      <div className="min-h-screen bg-slate-deep">
        ...
        <div className="max-w-6xl mx-auto md:ml-64 md:max-w-[calc(100%-16rem)] lg:max-w-5xl">
          <div className="sticky top-14 z-20 bg-slate-deep/95 backdrop-blur border-b border-slate-border h-12 flex items-center px-4 gap-3">
            ...
            <span className="text-cream font-medium text-sm truncate flex-1">{(listing as any).title}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-0 lg:gap-6 px-0 lg:px-4 pt-0 lg:pt-6 pb-24">
            <div className="bg-slate-card lg:bg-transparent lg:border-0 border-b border-slate-border lg:rounded-2xl overflow-hidden">
              <div className="aspect-[4/3] bg-slate-deep relative">
                <img src={images[activeImage]} alt={(listing as any).title} className="w-full h-full object-contain" />
                {(listing as any).expires_at && <div className="absolute top-3 left-3"><ListingCountdown expiresAt={(listing as any).expires_at} /></div>}
            ...
            <div className="px-4 lg:px-0 py-4 flex flex-col gap-4">
              <div className="bg-slate-card border border-slate-border rounded-2xl p-4">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <h1 className="text-cream font-bold text-xl leading-tight flex-1">{(listing as any).title}</h1>
                  <p className="text-cream font-bold text-xl flex-shrink-0">R{Number((listing as any).price).toFixed(0)}</p>
                </div>
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className="bg-slate-deep border border-slate-border text-cream-muted text-xs px-2.5 py-1 rounded-full flex items-center gap-1"><Tag size={12} />{(listing as any).category}</span>
                  {(listing as any).is_negotiable && <span className="bg-gold/10 border border-gold/30 text-gold text-xs px-2.5 py-1 rounded-full font-bold">Negotiable</span>}
                  <span className="bg-slate-deep border border-slate-border text-cream-muted text-xs px-2.5 py-1 rounded-full flex items-center gap-1"><Package size={12} />{(listing as any).condition || 'Used'}</span>
                  <span className="bg-slate-deep border border-slate-border text-cream-muted text-xs px-2.5 py-1 rounded-full flex items-center gap-1"><MapPin size={12} />{(listing as any).residence || 'Campus'}</span>
                </div>
                <p className="text-cream-muted text-sm leading-relaxed whitespace-pre-wrap">{(listing as any).description}</p>
                <div className="flex items-center gap-2 mt-4 text-cream-muted text-xs"><Clock size={12} /> Posted {new Date((listing as any).created_at).toLocaleDateString('en-ZA')} • {(listing as any).views || 0} views</div>
              </div>

              {seller && (
                <div className="bg-slate-card border border-slate-border rounded-2xl p-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0" style={{ backgroundColor: seller.avatar_color }}>{seller.avatar_initials}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-cream font-bold text-sm truncate">{seller.full_name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        {seller.avg_rating > 0 ? (
                          <button onClick={() => navigate(`/profile/${seller.id}`)} className="flex items-center gap-1 text-gold text-xs hover:underline">
                            <Star size={12} className="fill-gold" />{seller.avg_rating} · {seller.total_ratings} rating{seller.total_ratings !== 1 ? 's' : ''}
                          </button>
                        ) : (
                          <span className="text-cream-muted text-xs">New seller • No ratings yet</span>
                        )}
                  ...
                  <div className="flex items-center gap-2 mt-3 text-[11px] text-cream-muted"><Shield size={12} className="text-teal-light" />Meet on campus, check item before paying. Ratings are for sellers, not listings.</div>
                </div>
              )}
