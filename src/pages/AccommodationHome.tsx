...
export default function AccommodationHome() {
  ...
  const maxProperties = plan === 'accommodation_free' ? 1 : plan === 'accommodation_featured' ? 3 : 10
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-7">
        <div className="bg-slate-card border border-slate-border rounded-3xl p-6 sm:p-8 mb-7">
          <div className="flex flex-wrap items-start justify-between gap-5">
            ...
            <button onClick={() => navigate('/accommodation/plan-select')} className="inline-flex items-center gap-2 bg-teal-primary text-white font-bold px-5 py-3 rounded-xl"><Plus size={17} /> Add accommodation</button>
          </div>
          <div className="grid sm:grid-cols-3 gap-3 mt-7">
            ...
            <div className="bg-slate-deep rounded-2xl p-4"><p className="text-cream-muted text-xs">Properties</p><p className="text-cream font-bold mt-1">{listings.length} / {maxProperties}</p></div>
