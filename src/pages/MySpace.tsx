  ...
  return (
    <div className="px-4 mt-4 flex flex-col gap-4">
      <div className="px-4 sm:px-6 lg:px-8 pt-5">
        <h2 className="text-xl sm:text-2xl font-bold text-cream leading-tight">{getGreeting()}, {firstName} 👋</h2>
        ...
        <div className="flex flex-col lg:flex-row gap-5 items-start">
          <div className="flex-1 w-full min-w-0">
            ...
            <div className="flex flex-row gap-4 overflow-x-auto scrollbar-hide pb-2 snap-x snap-mandatory" style={{ WebkitOverflowScrolling: 'touch' as any }}>
              {([
                { title: 'Plan', tabs: ['Deadlines', 'Timetable', 'Schedule'] as Tab[], tile: 'bg-blue-50 border-blue-100 hover:border-blue-300 focus:ring-blue-100', iconBg: 'bg-blue-100', iconText: 'text-blue-600' },
                { title: 'Money', tabs: ['Budget'] as Tab[], tile: 'bg-green-100 border-green-200 hover:border-green-300 focus:ring-green-100', iconBg: 'bg-green-200/70', iconText: 'text-green-600' },
                { title: 'Focus', tabs: ['Pomodoro'] as Tab[], tile: 'bg-violet-50 border-violet-100 hover:border-violet-300 focus:ring-violet-100', iconBg: 'bg-violet-100', iconText: 'text-violet-600' },
                { title: 'Personal', tabs: ['Watchlist', 'Notebook'] as Tab[], tile: 'bg-pink-100 border-pink-200 hover:border-pink-300 focus:ring-pink-100', iconBg: 'bg-pink-200/70', iconText: 'text-rose-600' },
              ]).map(group => (
                <div key={group.title} className="w-[220px] shrink-0 snap-start bg-white border border-slate-border rounded-2xl p-3 flex flex-col gap-2 transition-transform duration-200 ease-out hover:-translate-y-0.5">
                  ...
                  <div className="flex flex-col gap-2">
                    {group.tabs.map(t => {
                      ...
                      const Icon = meta.icon
                      return (
                        <button key={t} onClick={() => setTab(t)} className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl text-[13px] font-medium text-left transition-all border text-slate-700 hover:text-slate-900 focus:outline-none focus:ring-2 ${group.tile}`}>
