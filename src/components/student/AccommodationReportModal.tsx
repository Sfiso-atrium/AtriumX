...
import { reportAccommodationListing } from '../../services/dataService'
...
export default function AccommodationReportModal({ accommodationListingId, onClose }: Props) {
  ...
  const handleReport = async () => {
    ...
    setLoading(true)
    const { error } = await reportAccommodationListing(
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
      <div className="bg-slate-card border border-slate-border rounded-2xl p-6 w-full max-w-lg">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-cream font-bold text-lg">Report Accommodation</h2>
        ...
        <p className="text-cream-muted text-sm mb-4 leading-relaxed">
          If this accommodation listing is fraudulent, misleading, or violates community rules,
